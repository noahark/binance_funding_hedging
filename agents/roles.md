...
|---|---|
| `gemini-3.7-flash` (the `agy` / `agy-review` windows) | `google` |
...
Antigravity is a CLI wrapper, not a provider. The `agy` and `agy-review`
window labels are assigned to `gemini-3.7-flash` (`google`), but if a session
runs another vendor's model, its provider identity is that vendor's. Name the
model, never the window label, wherever provider identity or isolation is
decided.
...
- `gemini-3.7-flash` (the `agy` window) may provide initial draft reviews.
  Its formal Review-1 work requires Human to name it in the dispatch and is
...
- Eligible Bookkeepers are `gemini-3.7-flash` (the `agy` window), Codex/GPT, and
  Claude.
...
- `status.json.bookkeeper` records the selected canonical model ID. Every
  ...
  or `gemini-3.7-flash`), not a terminal label such as `codex`, `claude`, or
  `agy`.
