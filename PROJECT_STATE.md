...
- **[2026-08-07] 现货符号解析改显式映射表 + P1/P2 死区修复已提交（`8ee6d3c`）**：
  ...
  - `[RESOLVED][2026-08-07]` Q4 统一账户可转出额：~~划转界面用 `cross_margin_free`
    ...
    直接进下拉标签）。
    **请求策略（Human 关注 API 频率，实测后定稿）**：**打开下拉 0 请求，选中某币 1 请求**。
    - 币安 maxWithdraw **无批量版**，一资产一次签名请求。**实测**单次往返 0.35~0.90s，
      18 个资产串行 **9.68 秒**——点开下拉干等 10 秒不可接受。**瓶颈是延迟不是权重**
      （18 次 ≈ 90 权重，占 PAPI IP 池约 1.5%）。
    - **USDT 不调接口**：走快照里现成的 `total_available_balance_usdt`。
      **实测同时刻对照**：该字段 209.18 vs `maxWithdraw(USDT)` 209.28（差 0.05%，
      ...
      成立的原因是 **USDT 是计价单位**——「账户还能动 209 USDT 的价值」对 USDT 而言
      就是「能转 209 个 USDT」。**严禁推广到其他币**：转走 BNB 时它作为抵押品的贡献
      一并消失，而抵押品按**折算率**而非市值计入保证金，「总额 ÷ 币价」必然偏大，
      偏大意味着以为能转、实际被交易所拒。
      附带收益：USDT 不依赖该接口，接口 503 时它照样准。
    - 其余币未选中时显示「可转 —」，选中才查那一个（约 0.5s）。
    - 端点保留批量形状（`assets=` 逗号列表 + 逐资产失败隔离 + 去重 + 30 个上限），
      前端目前只传一个：形状统一，后端已实现，无需为此回改。
    - **不要把触发点放回渲染函数**：曾放在 `renderAssetTransfer` 里，被 self-check 的
      「隐私切换不得 fetch」守卫抓住——面板 60s 自动重渲染、隐私开关等纯本地操作都会
      走渲染函数。渲染函数不得有副作用。
    **显示与校验口径刻意分离**：显示只认真实可转出额，读不到就是「可转 —」，
    **绝不回退 `cross_margin_free` / `total_balance` 顶替**（那正是本项要修的毛病）；
    校验用 `limit`——有可转出额用它，缺失时退回 `cross_margin_free`，否则「读不到」
    会连超额拦截一起失效。用可用额冒充可转出额去**显示**是错的，拿它当**校验上界**
    只是保守。现货账户仍说「可用」（`free` 本身就是可转出额，无抵押约束）且不请求。
    **未做**：真正的拦截仍以服务端/交易所为准，前端校验只挡明显不合理的输入。
