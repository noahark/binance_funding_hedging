...
- `[OPEN][HUMAN-FEEDBACK][2026-08-04]` **流水日志改为费率行情页内双看板 tab（布局迭代，不触 rework_count）**。任务 C（真实 API 接入）功能获 Human 认可「大部分 ok」，但布局改为：流水日志不再独立整页视图，费率行情页右上角展示「费率行情 | 流水日志」两个菜单、同页切换两个看板；侧栏 nav-flow-log 保留同级菜单。修订任务 `frontend-flow-log-tab-layout-v1`（grok）已路由；确认后设计落 v1.4 → 前后端联调（真实 `POST /refresh` 须 Human 授权）→ 统一 review-1 + review-2（A+B+C）。
