...
- **[2026-08-07] 现货符号解析改显式映射表 + P1/P2 死区修复已提交（`8ee6d3c`）**：
  ...
  - `[RESOLVED][2026-08-07]` Q4 统一账户可转出额：~~划转界面用 `cross_margin_free`
    ...
    直接进下拉标签）。
    **按需实时读，不进快照**：币安该接口**无批量版**，N 个资产 = N 次签名请求（实测
    统一账户 18 个），且值随价格波动，缓存住就会在按「划转」那一刻给出过期的数。
    循环放在**后端**，前端只发一个请求（否则是 18 个并发）；单资产失败不连累其余。
    **触发点是资产下拉的 `focusin`，不是渲染函数**——放渲染里会让 60s 自动重渲染与
    隐私开关等纯本地操作都打交易所（self-check「隐私切换不得 fetch」守卫抓到过）。
    20s 节流 + 失败也计入负缓存（防端点持续 503 时每次点开都打一轮）；选中某个资产时
    绕开节流单独强制重拉那一个（那是马上要转的数）。
