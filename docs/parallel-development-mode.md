...
delivery_mode: interactive_paste | in_session_execute | automation_cli | executor_self
delivery_instruction: <主交付指令：粘贴 PROMPT BODY、读文件执行 PROMPT BODY，或本会话直接执行>
automation_cmd: <可选；CLI 附录或 R10 self-invoke 命令>
adapter_cmd:   <deprecated alias；旧 packet 兼容，按内容映射为 delivery_instruction 或 automation_cmd>
started_at:    <ISO8601>
...
执行者只需按 `next_dispatch` 链找到文件并按 `delivery_mode` 执行，全链路
可审计、可断点续跑。新 packet 必须把 `delivery_instruction` 作为主入口；
CLI 只放在 `automation_cmd`，且只用于 runner 自动化、证据复现、可用性检查，
或预写并声明的 `executor_self` 自动化。
...
（目标会话 + dispatch 文件路径 + 可粘贴交付指令）。标注 self 的 packet
可以使用预写 `automation_cmd`，但该例外不得外推为普通人工 dispatch 的
CLI-first 规则。PROMPT BODY 与本文档规则冲突时以本文档为准，冲突本身按
设计缺陷升级 bookkeeper，不得按字面执行了事。
...
The prohibition is against **unlogged chat-window relay**. Prepare-only sessions
may prepare the dispatch file and optional automation appendix, but they must
not invoke another model terminal. The default human action is to deliver the
prepared `delivery_instruction` into the selected target terminal. If the
selected target session receives the dispatch path or PROMPT BODY, it executes
in-session and must not emit a same-provider/model relaunch wrapper. Declared
`executor_self` checkpoints may run the prewritten `automation_cmd`; that action
must be driven by the dispatch file and recorded in the required `.dispatch.md`
evidence.
...
5. **试运行 finding #1**（2026-07-04，stage `2026-07-phase2-borrow-sort-v1`）：
   ...
   等待嵌入预审 round 1」+ 预审 receipt 交付字段写"用户粘贴"，三处
