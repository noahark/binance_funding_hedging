...
> **§影响 所述的 1000x 数量错配风险仍然客观存在，但换算需求已于 2026-08-15 由 Human
> 决定不做、需求封存。** 六个乘数币继续由建任务 fail-closed 拦截挡住，
> **该拦截已由「止血」转为长期终态，零风险敞口**；相关脚手架不得作为死代码清理。
> 权威记录见 `PROJECT_STATE.md` 的 `[CLOSED-NOT-DOING]` 条目；
> 重启前必读 `docs/planning/leg-unit-size-conversion-2026-08-15.CLOSED-lessons.md`
> （含五轮评审经验与已知漏项——封存的改造清单**已知不全**，勿照其开工）。
