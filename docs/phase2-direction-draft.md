...
3. **密钥管理**：
   ...
   - Binance 侧 key 权限仅勾选「读取」（Enable Reading），不开现货/杠杆/合约
     交易权限；建议绑定 IP 白名单。
   - **用户决定（2026-07-04，覆盖 Fable5 的收权建议）**：保留现有 key 权限
     （含现货/杠杆/股票交易、杠杆借币还款划转、统一账户交易、子母账户划转、
     万向划转），**IP 白名单已绑定**。Fable5 曾建议换用仅读取 key，用户
     决定不换；风险披露：IP 白名单缓解 key 外泄滥用，但不约束白名单机器上
     代码自身的行为。**后果**：代码层端点白名单由纵深防御升级为**主防线**——
     (a) `private_client` 的白名单强制为 review-1/review-2 必查项；
     (b) 必须有单测断言白名单外端点（含任意 POST/PUT/DELETE）在客户端层
     直接 raise，测试输出入证据；(c) 启动自检不再断言「key 无交易权限」
     （必然失败），改为仅验证读通道可用 + 记录 `private_channel` 状态。
   - 后端启动时做一次**读通道自检**：调用一个白名单内只读端点验证 key 可用，
     并在 snapshot 元数据中记录 `private_channel: enabled/disabled`（不记录 key
     任何片段）。key 缺失时优雅降级为 Phase 1 行为（公开数据 + unverified）。
...
**用户确认（2026-07-04）**：账户为**统一账户模式（非统一账户专业版）**，
现货与合约账户存有少量资金。→ Phase 2b 账户级验证按统一账户端点族
（`/papi`）设计；Phase 2a 市场级不受影响。
...
1. ~~账户类型~~ **已确认**：统一账户模式（非专业版）→ 2b 走 `/papi`。
   剩余子问题：2a/2b 切分是否成立，还是直接按统一账户一步到位？
   （注意统一账户下经典 `/sapi` 杠杆清单作为市场级信号的适用性需评审确认。）
2. 安全基线（§3）是否充分？**注意用户已决定保留全权限 key（IP 白名单
   已绑定），代码层白名单为主防线**——在此前提下 §3.3 列出的三项后果
   （评审必查、单测断言、读通道自检）是否足够？是否需要追加（如签名
   请求全量审计日志）？
...
- DRAFT-1.1 (2026-07-04 18:35 CST, Fable5): 落档用户确认——账户类型为
  统一账户模式（非专业版）；现有 key 权限超配（含交易/借币/划转），
  新增「换用仅读取 key + IP 白名单」为 Phase 2 启动前置条件（§3.3）。
- DRAFT-1.2 (2026-07-04, Fable5): 用户决定保留现有 key 权限（IP 白名单
  已绑定），覆盖收权建议。§3.3 前置条件改为用户决定记录 + 三项代码层
  加固后果；自检降级为读通道验证；§8 Q2 相应更新。
