...
3. **密钥管理**：
   ...
   - Binance 侧 key 权限仅勾选「读取」（Enable Reading），不开现货/杠杆/合约
     交易权限；建议绑定 IP 白名单。
   - **启动前置条件（2026-07-04 用户现有 key 权限核查后新增）**：用户当前
     key 开有现货/杠杆/股票交易、杠杆借币还款划转、统一账户交易、子母账户
     划转、万向划转——远超需要且账户有资金。Phase 2 开工前必须换用
     **仅读取权限**的 key（新建或收权现有 key）+ IP 白名单。全权限 key 不得
     注入本工具环境。权限自检（下条）需断言 key 无交易权限，作为可核证据。
