# 开发方案 r4：两腿数量换算（每腿乘数口径）

- 作者：Opus 5
- 日期：2026-08-15
- 状态：**设计稿 r4，未实现。r1 双评审 REWORK → r2；r2 经 grok 评审 REWORK（claude-glm 因 provider 内容过滤 400 未能完成）→ r3；r3 经 Human 提问与作者自查 → 本稿 r4。**
- 目标读者：Human（不读代码）+ 评审模型
- 基线：`main` @ `f00df2b`（行号已于该基线实跑复核，§8.1 校验脚本 **33/33 OK**）
...
- 实现约 15 个改动点、跨 6 个文件、约 400–600 行（含测试）
...
**已知的收益脆弱点（供评审与 Human 权衡）：** 收益高度集中于 `1000XEC`（74%）。
若该标的下架或费率回归常态，本轮收益立刻塌去四分之三，而工程量一分不减——
**放行 1 个币与放行 6 个币的代码完全相同。**
...
### 2.6 哪些数字换算、哪些不换（r4 新增口径）

由 §3.5 的改法引出，须写死成一条规则，否则实现者只能逐个猜：

> **要拿去与交易所对账的数字，保持交易所原样（只加单位标注）；
> 只用于两腿互相比较的数字，才施加乘数。**

依据：§8.3 的实盘验收**要求 Human 拿系统显示的数字去币安页面逐项核对**。若把持仓数量
换算成「个 BONK」，就与币安页面对不上，而对得上恰恰是本轮最需要人肉验证的东西
（记账与计算共用同一套假设，自证无效）。反之两腿均价这类只用于互相比较的量，
不换算就毫无意义（`0.000012` 与 `0.012` 并列展示对读者零价值）。

| 类别 | 例 | 处置 |
|---|---|---|
| 对账数字 | 持仓表持仓数量（`um_position_amt` / `position_qty`）、发给币安的 `quantity` | **不换算**，加单位标注 |
| 比较数字 | 两腿均价、两腿价差率、平滑覆盖率、盘口开单率 | **换算**后再比较 |

⚠️ 该规则**请评审重点核对**：若存在既要对账、又要跨腿比较的量，本规则会自相矛盾，
需在评审阶段暴露。
...
| 3 | `store.py:2834-2836` `aggregate_positions` 的 `perp_avg` | 后端成交记录 | 持仓表「合约均价」列与「开单价差率」列的**共同数据源** | **日常最显眼的一处**（详见下方 r4 改法） |
...
**第 3 项的改法（r4 变更，替代 r3 的「前端加乘数」）：**

r3 原方案是给前端 `computeHedgeOpenBasisRate`（`index.html:5249`，调用点 `6624`）
加乘数。**r4 改为在后端源头换算**，依据是一次实测核查：

```
perp_avg 的全部消费者（grep 确认）：
  frontend/index.html:6731   合约均价列          ← 纯展示
  frontend/index.html:6624   开单价差率计算入参   ← 纯展示
后端消费者：零。不经过任何下单、风控、对账路径。
```

→ 因此在 `store.py:2834-2836` 算出 `perp_avg` 的**同一处**直接 `× x` 后输出。收益：

| | r3 改法 | r4 改法 |
|---|---|---|
| 合约均价列 | 仍显示 `0.012`，与现货 `0.000012` 不可比 | 显示 `0.000012`，**与现货同量纲可直接对比** |
| 开单价差率 | 改 JS 公式 | **前端一行不改**，自动 ≈ 0 |
| `self-check.js:5110-5117` 两条断言 | 需重写 | **原样通过**（倍数 = 1 路径未变），回归证据保住 |
| 前置轮「改动二」 | 「反正要碰这个 JS 函数」 | **该前提消失**，故 §6 判定不并入 |

⚠️ **此改法须评审重点核对两点**：(a) `perp_avg` 确无后端消费者；
(b) 该换算是否与 §2.6 冲突——本稿判定不冲突（`perp_avg` 属「比较数字」，
持仓数量 `um_position_amt` / `position_qty` 属「对账数字」，两者在同一张表里但类别不同）。
...
理由是「乘数轮无论如何要碰 `index.html:5249` 这段代码」。**该前提已被 §3.5 的 r4 改法消除**
——本轮在后端源头换算 `perp_avg`，**根本不碰那个 JS 函数**。
...
> r4 在 r3 的 33 条基础上补入 13 条（reverse 分支、`aggregate_positions` 的 `perp_avg`、
> `fakes.py` 的 `OutcomeSpec` 与两处模拟成交调用、绕拦截夹具、前端两个消费点）。
> 基线 `f00df2b` 实跑 **46/46 OK**。
...
| 阶段 | 内容 | 验收 |
|---|---|---|
| **A** | **A1** 假执行器成交模拟拆两腿价格（`fakes.py` 单文件，照抄 `OutcomeSpec` 现有 `spot_qty`/`perp_qty` 形状，默认值不变）<br>**A2** 写红测试（§8.2 用例表），用现成夹具 `_allow_multiplier_open`（`test_hedge_cycle_close.py:1199`）绕过拦截 | **新测试红、旧测试全绿**（除已知基线失败 `test_urlopen_only_in_designated_http_clients`） |
...
| **D** | §3.2–3.6 **一批改完，不拆分交付**：开仓 6 处 / 平仓 2 处 / 平滑 2 处（含 reverse）/ 展示 3 处（含 reverse）/ UI 文案 3 处 | A2 **全部**红测试转绿 |
...
> r1/r2/r3 的历史问题已逐轮回答，见 §10 与文末附录。**r4 只问本轮新增或变更的部分。**
> 作者已自行完成 r3 §11 问题 1 的横扫，结论写在问题 1 里，请**独立复核而非采信**。

1. **§3.4 / §3.5 的 reverse 分支是否补全？还有没有第五处？**
   作者自查结论：「两腿价差」确为 3 份独立实现（盘口 / 后端成交 / 前端成交），
   r3 的三处判断成立；**但每处的 reverse 分支未被列名**（`domain.py:1591`、
   `snapshot.py:699`），r4 已补。同时核查并排除了三项：`service.py:1822` 的滑点门
   路由进 `evaluate_smooth_gate`（§3.4 已覆盖）、`self-check.js` 只有 fixture 无独立公式、
   `aggregate_positions` 的 `open_basis_rate` 硬编码 `"0"` 且前端不读。
   **请独立复核，并搜索是否仍有第四份实现或第五个分支。**

2. **§3.5 第 3 项的 r4 改法（后端源头换算 `perp_avg`）是否安全？**
   依据是「`perp_avg` 的消费者只有前端两处、纯展示、后端零消费」。
   请独立验证该 grep 结论；并判断是否存在通过 API 契约、导出、报表、日志间接消费该字段
   而作者未搜到的路径。**若该前提不成立，本改法会把一个展示问题变成数据问题。**

3. **§2.6 的规则（对账数字不换算 / 比较数字换算）是否自洽？**
   是否存在**既需与交易所对账、又需跨腿比较**的量？若存在，该规则自相矛盾，须在实现前
   暴露。特别请核对持仓表：`perp_avg`（按本规则换算）与 `um_position_amt` / `position_qty`
   （按本规则不换算）同处一行，读者是否会被误导。

4. **§2.5 的 `face_value` 判断是否成立？**
   本稿主张币安 `face_value ≡ 1`、本轮 `y = 倍数` 直接成立、将来叠乘即可，**故不建任何机制**。
   请核对该主张对币安 U 本位永续是否普遍为真（含 `1000000` 前缀、bStock `TRADIFI_PERPETUAL`），
   以及「不留占位」是否会在将来造成不可逆的返工。

5. **§9 的排期（A 阶段先行）是否成立？**
   依据两条实测事实：`fakes.py` 运行时不可达（`service.py:3665`）、`OutcomeSpec` 已有两腿
   各自数量字段。请核对；并判断 A1 的默认值设计能否真正做到「现有 1939 条测试零影响」。

6. **§0.5 的成本/收益是否改变结论？**
   实测边际贡献 1.72%，且 74% 集中于单一标的 `1000XEC`。作者结论是「值得做，但理由是
   修 bug + 拆脚手架（净新增维护面约 3 处），不是这 1.72%」。
   **请独立判断：这是否足以支撑一轮触及下单数量路径的改动？** 若认为不值得，请直说。

7. **本稿是否还有引用错误？** r1 有一处（`_is_close_flat` 不存在），r2 有一处文档自相矛盾
   （`2133`），r3 有两处漏列分支。**请实跑 §8.1 校验脚本并报告结果**——r4 已把本轮新增的
   13 条引用一并纳入该脚本，作者于基线 `f00df2b` 实跑为 **46/46 OK**。
   校验脚本只证「行号命中」，**不证「改法正确」**，后者仍需你独立判断。
