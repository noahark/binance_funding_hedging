...
状态：**设计 r2（第一次计划评审后的返工稿），等待第二次跨 provider 只读计划评审。未授权改源码、重启服务、创建任务、下单、push、merge 或部署。**
日期：2026-08-14（r1 → r2 同日修订，修订依据见 §11）
...
| C5 **（r2 修订）** | 备料时机：**在 `post_start` 接口内同步执行**，全部成功后才把任务置为 `running` 并启动 worker、订阅盘口、建立 gate | r1 把备料放在 worker 启动后、订阅之前（异步），失败后卡片会先跳到"执行中"再消失 | 卡片在备料期间原地停留在"已暂停"，前端按钮显示"备料中…"；成功才转"执行中"。失败由 HTTP 响应直接带回中文原因当场弹出。worker 模型一行不改，只是把备料挪到状态翻转之前 | Human 2026-08-14 决定 |
...
| C10 | 5 分钟固定窗口、`market / manual / timeout` 三种放行原因、gate 身份 `(task_id, next_attempt_seq)`、`成交1次` 携带 gate_seq、不提供「立即成交所有」——全部逐字沿用平滑开单 | — | 镜像要求。这些语义已由平滑开单 V1 的持久化 gate + `prepare_attempt` 原子复核封住三方竞态与超发 | Human 镜像要求 |
| C11 | 80% 一档覆盖率的分母 = 备料冻结的 `q_common` | 曾考虑直接用输入的 `single_amount` | §2.2：`q_common` 是取整后的实际发送量，步长粗的币两者可差一整格 | 当前代码事实 |
| C12 | 平仓闸门接入平滑链：`put_close_gate` 唤醒等待中的 gate；gate 等待循环检查 `is_close_gate_on()` 并在关闸时 `clear_smooth_gate`；**`_dispatch_one_for_task` 的发单准入同时要求平仓闸门开启** | 现状三处都没有（§2.6） | 否则关闸后仍存在长达 5 分钟的"照发一笔"窗口。该窗口与开单 L1 的微秒级竞态不同量级，不适用 L1 的已接受结论 | 落实 C10 的最小实现约束（r2 补齐发单侧） |
| C13 **（r2 新增）** | 启动交互：点击启动 → 卡片**原地停留在"已暂停"**、按钮显示"备料中…" → 备料成功才转"执行中" → 备料失败当场弹出中文原因且卡片状态不变 | r1 的异步备料会让卡片先跳"执行中"再消失回"已删除"，操作员以为"点了没反应" | 与 §2.5 的建卡同步划转交互一致。**代价（Human 明确接受）**：启动请求会真实等待数秒（预检读取 + 可能的划转 + 100ms），交易所慢时可能十几秒，前端必须给明确等待态 | Human 2026-08-14 决定 |
| C14 **（r2 新增）** | `q_common` 只在**三步全部成功之后**写入；写入与置 `running` 属同一次启动流程的结尾 | r1 的顺序是"预检出数量即冻结 → 查可平量 → 划转"，且以 `q_common` 非空作为已备料判据 | 否则在"已写数量、未划转"之间崩溃时，恢复会认为已备料而跳过划转，现货腿按无币去卖（必挂 + 合约腿成交 = 单腿裸敞口）。改为末步写入后，任何中断都只退回"未备料"，下次启动重做且划转幂等 | Human 2026-08-14 决定（评审 grok 指出） |
| C15 **（r2 新增）** | **没有有效 `q_common` 不得建立 gate、不得订阅、不得放行**（含 timeout 与 manual）；该状态下的放行请求 fail-closed | 现状：数量无效只让行情不通过，timeout/manual 仍会走下单链，且 `send_qty` 回退成未取整的 `single_amount`（§2.2） | 封死平滑开单 F-A 在平仓侧的同形路径。C5 的同步备料使该前提天然成立，本条是防止实现把顺序排反的硬不变量 | Human 2026-08-14 决定（评审 grok 指出） |
...
（建卡，不变）close 轻量建卡：活跃周期校验 → 身份继承 → 1000x 拒绝
        ...
        ↓
（Human 点「启动」）POST /start —— C5/C13：备料在本请求内同步执行
        ├─ 已有有效 q_common？→ 是：跳过备料（人工暂停后恢复的路径，C4）
        └─ 否：fresh preflight → 合约可平量门 → forward 现货余额检查/划转/sleep(100ms)
                ├─ 任一失败 → 任务保持/回退 paused + §2.4 既有中文原因
                │             HTTP 返回该原因；零订阅、零 gate、零 attempt、零订单
                └─ 全部成功 → 【C14】此时才写入 q_common / position_side_mode /
                              preflight_snapshot(route) → 置 running → 启动 worker
        ↓
        worker：订阅现货/合约两个 watcher → 建立或恢复 gate（【C15】无有效 q_common 不得建立）
        ↓
（每一轮，第 1…N 轮完全相同）
...
6. 平滑开单 V1 已具名接受、平仓侧同样继承的三项限制：L1（Start gate 关闸与放行之间的微秒竞态）、L2（新 gate 可能使用上一轮结算前的 `now_us` 导致窗口略短）、L3（前端整表刷新会重置未提交的阈值输入——持仓表加阈值输入后同样作用于平仓列）。本轮**不修**，不得顺手扩大受审范围。**注意平仓闸门不在此列**：§2.6 的窗口长达 5 分钟，由 C12 必修。
...
- `post_start` 对 `smooth + close` 且 `q_common` 为空的任务：先同步备料，全部成功后在同一流程结尾写入 `q_common` / `position_side_mode` / `preflight_snapshot`，再置 `running` 并 `ensure_worker`；
- 备料任一步失败：任务保持 `paused` 并写入 §2.4 的对应中文原因，HTTP 返回该原因（沿用既有错误响应形状），**不置 running、不启动 worker**；
- `q_common` 非空时跳过备料，行为与今天的 `post_start` 逐字一致（C4）；
...
- 启动按钮在请求进行中显示"备料中…"并禁用重复点击；失败时原地展示后端返回的中文原因（C13）；
...
10. **无数量不放行（C15）**：构造 `q_common` 为空且 status 为 running 的任务，gate 不得建立；即使注入 timeout 到期或 `成交1次`，`prepare_attempt` 与 executor 的调用计数仍为 0，绝不出现以 `single_amount` 发送的两腿请求。
...
13. **平仓闸门（C12）**：等待中关闭平仓闸门 → gate 被立即唤醒、清空、worker 退出；**且在关闸后触发 timeout 放行时，`_dispatch_one_for_task` 不得发单**；重新开闸后为仍未调度的同一 seq 建新的完整窗口。
...
21. **延迟审计**：平仓放行同样产出 `smooth_dispatch_audit`，快照来自产生放行结论的同一次读取；写入发生在两腿 executor 返回后，失败不改变订单处置。
