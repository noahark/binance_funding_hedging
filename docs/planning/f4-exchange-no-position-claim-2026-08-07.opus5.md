...
| `no_um` 文案降调（§8-3） | **已做**（Human 追加授权），见下 |
...
### 11.1 §8-3 文案降调（Human 追加授权后补做）

原本以为是一行文案，动手时发现它对**一整类行完全不适用**，故成了「一行文案 + 一个条件」：

**已平仓周期行（`cycle_closed_at` 非空）本来就该没有交易所仓位**——那是预期结果，
不是异常。此前它会与「已完全平仓」标记**并排**印出红色「交易所无仓（可能已强平）」，
对一个正常平完的周期是纯误导。现在只有**活跃周期**却在交易所找不到仓位才警示。

文案同步降调：

```
旧：本地有任务记录，但交易所无对应持仓（可能已强平或手工平仓）
新：本地有活跃周期记录，但交易所当前无对应持仓
    （可能已强平、手工平仓，或本地记账与交易所不同步）——请到币安核实
```

即便真读到了、确实没这个仓位，「强平」也只是成因之一。不把推测说成结论。

**已验证会红**：注入回退（去掉 `!p.cycle_closed_at` 条件）→ self-check 报
「已平仓周期不该警示『交易所无仓』」；注入已还原。

至此 F4 的三条路径与 `no_um` 自身的措辞全部收口，PROJECT_STATE 里那条操作规矩
（「『交易所无仓』永远不足以证明仓位没了」）不再必要——但**仍建议保留**为习惯，
因为陈旧数据（§7-1）这个独立维度没有被本轮覆盖。
