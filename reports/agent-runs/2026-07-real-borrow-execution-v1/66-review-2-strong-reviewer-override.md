...
A fresh, human-dispatched Codex/GPT read-only session is selected as the
preferred strong reviewer. It did not implement or fix delivery code. Its
Review-2 prompt must treat the user-approved direction synthesis, product PRD,
and product architecture as higher authority than this stage's design and
breakdown; it must explicitly disclose `reviewer_prior_involvement: "design"`
in both narrative and strict JSON verdict.
...
下一步模型: Codex/GPT（fresh read-only Review-2 session）
