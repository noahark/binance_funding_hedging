...
2. 合成 fixture（`backend/tests/fixtures/bstock-alias-raw/`）新增至少一个
   USDC 计价 PERPETUAL（期货 `BTCUSDC`，现货同时含 `BTCUSDC` 与 `BTCUSDT`），
   新增回归断言：该行 `spot.symbol=="BTCUSDC"`、`match_type=="exact_symbol"`、
   绝不解析为 `BTCUSDT`；既有 bStock alias 与 USDT exact 用例不变。
3. 离线复验（禁止新的 live HTTP）：对已提交的 live raw 重放构建，断言不存在
   `match_type=="exact_symbol" && spot.symbol != futures.symbol` 的行；输出追加
   到 `60-test-output.txt`（新 Section）。
4. 撰写 `40-fix-report.md`：逐条 finding→fix 映射（finding id
   `spot_leg_quote_asset_hardcode` → 代码/测试/复验各自的落点与结果）。
...
- **不得顺手修** `quote_asset` const/universe 过滤问题（既有 impl-v1 问题，
  已记入 `status.json.open_items`，等用户决策；本轮 rework 不扩大范围）
