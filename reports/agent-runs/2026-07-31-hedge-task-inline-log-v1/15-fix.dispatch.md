...
3. **`resolve_attempt`**（`store.py:1104-1135` 一带）：解包新增值,`UPDATE hedge_open_leg`
   语句增加 `avg_price = ?`。
4. **`_row_to_leg`**（`store.py:258`）：增加 `"avg_price": row["avg_price"]`。
5. **`_leg_to_doc`**（`service.py:214`）：均价优先级改为
   **① 库里存的 `avg_price`（交易所权威值）→ ② 没有则退回既有的 `quote / base` 本地计算
   → ③ 都没有则 `None`**。
6. 补测试：覆盖三级优先级、migration 幂等、既有行不受损。
...
- `backend/hedge_open_tasks/store.py`（Part B：migration、`_leg_final_fields`、
  `resolve_attempt` 的 UPDATE、`_row_to_leg`）
- `backend/hedge_open_tasks/service.py`（Part B：`_leg_to_doc` 的均价优先级）
...
1. **列头已改**：日志表第三列为「尝试时间」;全量搜索证明代码中不再有「成交时间」字样。
2. **所有行都显示时间**：四种状态（进行中 / 已受理 / 已确认失败 / 单腿成交）各有用例,
   断言**每一行**都显示该次尝试的时间（北京时间格式）;仅当 `attempt.ts` 缺失才 `—`。
...
9. **回归**：`node frontend/self-check.js` 全过；`python3 -m pytest backend/tests -q` 全过
   （贴原始输出）。既有用例转红必须逐条解释原因——`avg_price` 是加性列,原则上不应有
   既有用例转红；若有,说明碰到了不该碰的地方,停下回报。
