...
`[start, end)`. Separately recognize successfully decoded JSON container spans
for both dictionaries and arrays. A dictionary candidate is nested when its
complete span is strictly contained by another decoded container span. Select
the final dictionary candidate among the non-nested spans. This prevents both
findings-array members and a verdict dictionary wrapped by a top-level array
from being promoted as a top-level verdict.
...
- Do not accept arrays or scalars as verdict objects. A dictionary contained
  by a decoded array remains nested even when that array is the outermost JSON
...
- Container tests: a valid verdict wrapped by one or more top-level arrays is
  not promoted; a normal verdict with a findings array still succeeds.
...
下一步任务: close BK-H-001 by treating decoded arrays as containing spans without weakening prose/footer compatibility
