# 平仓两段式交付 Review-2 dispatch — Kimi / Moonshot

> **Supersession（2026-08-09）：** 本 dispatch 取代同阶段未启动的
> `…review-2-sonnet5.dispatch.md`。Human 在启动 Review-2 前决定改派 Kimi（理由：Sonnet 5
> 能力不足以替代 Opus 5 做本轮 HIGH_RISK 现实核验；Kimi 是本 harness 首选的跨 provider
> 只读评审模型）。属 pre-dispatch packet 修正，不计 `rework_count`；sonnet5 版保留在 git
> 历史作为被取代的未启动 packet（见 `PROJECT_STATE.md` O-C follow-up 口径）。
...
- target_model: `kimi`
- provider: `moonshot`
- status_revision: `3`（本 stage `status.json` revision，对应 `current_task` 指向本 dispatch）
...
- 该范围不含本 stage 的控制/ledger 提交（dispatch、status.json、intake、B 证据、ACTIVE.json、
  Review-1 handoff、两份控制文稿、sonnet5 dispatch、本 dispatch）；针对控制提交的发现按 §8
  评审范围口径记为范围外。

## 隔离披露

- 实现作者：Codex / OpenAI（provider `openai`）；交付范围内无其它实现/修复作者。
- 本 Reviewer：Kimi（provider `moonshot`）。Review-2 须与交付范围内**全部**实现/修复作者跨
  provider：`moonshot` ≠ `openai`，隔离成立（`AGENTS.md` §3.5、`agents/roles.md` Reviewer/Isolation）。
- **三轮三 provider**：实现 `openai` → Review-1 `anthropic`（Opus 5）→ Review-2 `moonshot`（Kimi）。
  本 Reviewer 为独立只读新会话，**未参与**本需求的计划、设计、实现或 Review-1，作为最终评审者
  独立性最强（符合 §8「prefer a final reviewer that did not plan or design the stage」）。
- 默认 Review-2 模型原为 `sonnet5`（DEC-2026-08-04-001）；Human 本次显式改派 Kimi，模型选择属
  Human 决定。
...
- **唯一允许新建（create-only）**：
  `reports/agent-runs/2026-08-09-close-task-preflight-simplification-v1/evidence/close-task-preflight-simplification-review-2-kimi.handoff.md`
- Bookkeeper 预检（2026-08-09 17:11 CST）：`test ! -e <上述路径>` → **ABSENT**，create-only 权威成立。
...
1. `AGENTS.md`
2. 本 dispatch：`reports/agent-runs/2026-08-09-close-task-preflight-simplification-v1/close-task-preflight-simplification-review-2-kimi.dispatch.md`
...
2. **隔离**：实现作者为 OpenAI、本 Reviewer 为 Kimi/Moonshot；披露 Review-1（anthropic）与计划复评参与。
