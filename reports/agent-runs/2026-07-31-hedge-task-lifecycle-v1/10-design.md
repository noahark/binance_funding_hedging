...
> **增补（2026-08-01，`fix-merged-positions-mismatch-labels-v1` 修复轮 2/3）**：review-2（`43-` §1.1）发现首轮交付**整列删除了 fake 的「状态」列且未在 §5 列出**，属未披露的形状偏离。本轮恢复「标清楚」能力，下列四项同步补入差异清单，使「未列即一致」重新成立（dispatch Acceptance #8）：
> 6. **状态标识（恢复 fake 的「状态」语义，改用后端显式字段）**：fake 用独立「状态」列（`fkRowStatus`：正常/无任务记录/交易所无持仓/单腿敞口）；真实本轮**新增后端显式字段 `match_status`（normal/no_task/no_um）**，前端在既有「标记」列渲染简短标签「无任务记录」「交易所无仓」（推测原因放 `title`），单腿敞口维持既有标记。原因：后端合并层同时知 UM 侧/任务侧是否存在，显式字段比前端靠全零反推稳妥（靠全零反推正是本 stage 反复栽跟头的歧义）。**展示形状（列/六场景视觉/三分类）不变**——状态信息回到「标记」列而非新增列，保持表格紧凑。
> 7. **均价不完整标记（G5，新增）**：fake 无；真实对「成交金额为字面 `0`/`NULL`（未知）的腿」置 `*_avg_price_incomplete`，前端在「标记」列加「均价不完整」并在均价单元格加 `title`。原因：币安 2026-07-14 移除 UM 下单返回的成交金额，未知被存成 `"0"`，须与真值区分、不得拉低均价（`44-` §3）。
> 8. **全仓借款同币去重（G6，新增）**：fake 无；真实对同币多行按 base 资产去重，首行显示真值、其余行显示「同↑」，均带 `title`「账户级·按资产·勿竖向相加」。原因：全仓借款是账户级按资产数值，同币多行重复呈现会误导竖向相加（`44-` O-3）。
> 9. **均价小数位收敛（G7，新增）**：fake 无；真实「现货均价」「合约均价」两列统一收敛到 8 位有效数字（缺失仍 `—`、真实 0 仍 `0`）。原因：除法曾产生 27 位尾数，同列格式不统一（`44-` O-4）。
