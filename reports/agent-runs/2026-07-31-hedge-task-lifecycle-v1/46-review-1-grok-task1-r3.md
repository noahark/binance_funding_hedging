...
## 3. Human 裁定（2026-08-01）：F4 接受为已知限制，修复折入 Task 2

Human 最初的判断是「只在启动时出现，重启后数据没加载完，我不会当真」。Bookkeeper 核实后**纠正了该前提**（见 §3.1），Human 在知悉修正后的事实后决定：

> **接受为已知限制，Task 1 不再修；修复折进 Task 2 的范围。**

理由：`rework_count` 绑**交付物**，Task 2 是全新交付物、额度从零重算，且该修复属 Task 2 范围内的正常实现而非返工 —— 因此在 Task 2 里改**既不消耗 Task 1 的最后一次额度，也不消耗 Task 2 的**。

由此：

- `fix-match-status-account-unavailable-v1.dispatch.md` **作废**（从未交付）。
- **`rework_count` 由 `3` 回落为 `2`** —— 第 3 轮修复从未发生，§8 计数的是实际发生的修复轮。Task 1 仍余 1 次额度。
- Task 1 的交付**保持在 `ef53a02`**，代码不再改动。

### 3.1 Bookkeeper 对 Human 原判断前提的纠正（已被采纳）

误标有**两条**触发路径，只有一条是启动期：

| 路径 | 何时发生 | Human 原判断是否适用 |
|---|---|---|
| `SnapshotNotReady` | **仅**首次发布前的启动窗口 | ✅ 适用 —— 刚重启、数据未加载，用户确实不会当真 |
| `private_account.verified == false` | **任何时刻**。`fetch_unified_balances` / `fetch_spot_balances` 在 `PrivateEndpointError` 时返回 `None`（`private_client.py:559` 等），两者皆空即置 `verified: false`。触因包括 API key 失效、IP 白名单变更、币安私有接口报错、网络抖动 | ❌ **不适用** |

第二条才是风险所在：程序已正常运行数小时、数据一直为真，某一刻所有行突然变成「交易所无仓 / 可能已强平」—— 此时用户**没有任何理由怀疑它**，恰恰因为它此前一直正确。

### 3.2 已接受风险记录（`AGENTS.md` §8 五要素）

- **问题事实**：账户侧读不到时（`SnapshotNotReady` 或 `verified: false`），`match_status` 仍输出 `no_um`，前端渲染「交易所无仓」并附 `title`「可能已强平或手工平仓」。经实测，账户块内**确实含该 UM 持仓**时也照样如此；「查过没有」与「根本没查」不可区分。
- **可能影响**：用户可能误信仓位已被强平或已被手工平掉，进而做出错误的操作判断（例如误以为无需平仓、或重复开仓）。**不造成资金损失，但可能诱发错误操作。** 启动窗口内影响低（用户知道数据未加载）；**私有读中途失败时影响显著更高**（无从察觉）。
- **接受理由**：Human 判断启动期场景可自行辨识；中途失败场景虽真实存在，但修复可零成本折入紧随其后的 Task 2，无需消耗 Task 1 的最后一次返工额度、也无需阻塞 Task 1 的评审收口。
- **临时限制 / 观察方式**：**看到「交易所无仓」时，先确认页面上有没有「账户数据未就绪」横幅** —— 两者同时出现即为本限制，该行的匹配状态不可信，须以个人账户面板的真实 UM 持仓为准。横幅不存在时，「交易所无仓」才是真实结论。
- **后续复看条件**：**Task 2 交付时必须一并修复**（已写入 Task 2 的范围要求，见 §3.3）。若 Task 2 因故未做，本条须在合并 `main` 前重新交 Human 裁定。

### 3.3 转入 Task 2 的具体要求（Bookkeeper 待办：写入 Task 2 的 dispatch）

1. 契约：仅在账户侧已成功读取时才允许 `match_status = "no_um"`；`verified` 为假或 `private_account` 缺失时须用中性值（建议新增 `account_unavailable`，属枚举值不改键集）。
2. 展示：N2 路径不得出现「交易所无仓」标签与强平/手工平 `title`。
3. 测试（须能失败）：`verified=false` 与 `private_account=None` → `match_status != "no_um"`；`verified=true` + 无 UM + 有桶 → 仍为 `no_um`；self-check 断言 `verified=false` 时行 HTML 不含「交易所无仓」。
4. 同步 `21-` §11.2 的 `missing × 标记` 格与 `10-design.md` §5。

## ~~3-old. 返工预算：已达上限~~（已由 §3 撤销，第 3 轮未发生）
