...
- **汇总用 `Decimal` 精确求和**并输出 `format(total, 'f')`；**只要分组内有一行金额无法解析，该分组 `*_total` 必须为 `null` 且 `unparsed_row_count > 0`**——不得用部分和冒充完整合计。此规则同样适用于增量与今日累计分组。
...
- 一次写入批次的所有行共用同一个 `first_seen_at_ms`（服务端在事务内取一次时钟）；明细 + run 记录 + `ledger_meta` 在**同一事务**内提交，失败整体回滚。
- 提供窗口查询（时间倒序、可限制返回条数但**返回全量条数与全量汇总所需的行**）、覆盖范围读写、run 记录写入与「最近 N 次成功 scheduled run」查询——供任务 B 计算 §15.4 的增量基准。
- 永久保留，不实现任何自动清理。
...
- `docs/planning/2026-08-04-dual-ledger-flow-log-design.md`（§12–§18；本任务的冻结契约为 §13.2、§13.5、§13.6、§14、§15.4）
...
4. `store.py` 幂等可验证：同一行重复写入不产生第二条、**不覆盖** `first_seen_run_id` / `first_seen_at_ms`；同批次行共用同一 `first_seen_at_ms`；写入失败整体回滚；四张表与索引按 §14 建立。
5. 金额精度红线：金额列为 `TEXT`，代码中不存在对金额列的 `SUM`/`AVG`/算术 SQL，也无 `float()` 参与金额路径（以测试或静态断言证明其一）。
6. 边界未被越过：未新增 HTTP 路由、未启动线程、未改快照 schema / 60 秒调度 / cache-refresh / 持仓合并 / 任何既有端点行为；`backend/ledger_flow/__init__.py` 仅含 docstring。
7. 离线运行并把原始输出保存到 Allowed Files 中的 `.pytest.txt`：`PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -p no:cacheprovider -q backend/tests/test_ledger_flow_domain.py backend/tests/test_ledger_flow_store.py backend/tests/test_private_client.py`；全部以桩客户端与临时 SQLite 离线完成，不启动服务、不访问网络、不读凭据、不写 `data/`。
