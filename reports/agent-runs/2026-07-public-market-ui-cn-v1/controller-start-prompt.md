...
2. **Task A（你自己，senior_developer）**：按 00-task.md Task A 节执行——
   ...
   fingerprint。
3. **Task B（派发 Kimi，minimal_change_engineer）**：派工文案已由 Fable5
   预写：`task-b-kimi-frontend.prompt.md`，**原样发送**（不得删改技术约束；
   如 H_A 后有需要补充的上下文只能追加，不能替换）。Kimi 完成后提交 H_B，
   绑定 task-B fingerprint。
