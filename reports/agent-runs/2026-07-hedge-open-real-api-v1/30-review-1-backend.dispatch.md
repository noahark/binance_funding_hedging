...
你是正式 Review-1 的 Kimi K3 后端交叉评审者。只读审查，绝不修改文件、绝不 commit、
绝不触发真实 Binance 网络/私有请求/POST，也不要运行写入性命令。你不得评审或改写前端
任务。实现者为 claude_glm（zhipu_glm），所以你与实现者 provider 隔离。
...
- reports/agent-runs/2026-07-hedge-open-real-api-v1/{00-task.md,05-cadence-resolution.md,06-direction-synthesis.md,10-design.md,11-adr.md,12-development-breakdown.md,13-r4-diff-reconciliation.md,14-r4-verification.md,20-implementation-backend.md,40-fix-backend-r4.md,60-test-output.txt}；
...
model=kimi-k3，diff_fingerprint 必须是上面的 backend task fingerprint，
...
下一步模型: bookkeeper
下一步任务: validate this Review-1 verdict and route ACCEPT to the frontend cross-review or REWORK to a bounded human-dispatched fix
```
