...
- `backend/tests/test_private_client.py`（**scope amendment SCOPE-2026-07-09-002**：§1.1(b) 成功路径
  补 `error_code:None` 波及 3 处成功断言 `:216`/`:225`/`:262`（机械追加 `, "error_code": None`）。
  `:238`（-2015→`max_borrowable_failed`）**保持不变**。另加 1 个正数未知业务码用例。见 §5。）
...
- `fetch_max_borrowable` except 三分支（**分界依据 = 码的正负号**，见 ADR-2 修订）：
...
- ⚠️ **不得**把负数 Binance 码归入 business_error：负数=系统/鉴权失败（既有约定 `snapshot.py:27`/contract:72,301/
  `test_private_client.py:230-232`）。`test_private_client.py:238`（-2015→`max_borrowable_failed`）是金标准，**勿改**。
...
- backend 单测（就近 `test_private_client.py`/`test_private_account_v1.py`/`test_snapshot.py`，勿重写无关用例）：
  51061→"0"·**未知正数码**(如 51099)→None+`max_borrowable_business_error:`·**负数码 -2015**→None+
  `max_borrowable_failed:`(`test_private_client.py:238` 原样)·系统错误(无 code)→None+`max_borrowable_failed:`·
  成功→error_code None(`:216`/`:225`/`:262` 追加)·折算 8dp（"0"→"0.00000000"/缺价 None/stable)·
  assemble 三分支 additive 透传。
